...
function calcularApvParaBajarTramo(rentaAfectaSinApv, utmValor, ufValor) {
  ...
  // Tope legal Régimen B: 600 UF/año → 50 UF/mes
  const TOPE_UF_MES = 50;
  const topePesos   = TOPE_UF_MES * ufValor;
  const topado      = apvNecesarioUFReal > TOPE_UF_MES;

  const apvNecesarioUF    = topado ? TOPE_UF_MES       : apvNecesarioUFReal;
  const apvNecesarioPesos = topado ? topePesos          : apvNecesarioPesosReal;
  ...
  return {
    ...
    apvNecesarioUF,
    limiteEnPesos,
    topado,               // true si el cálculo real supera el tope de 600 UF/año
    apvNecesarioUFReal,   // valor real sin tope (para mostrar en el mensaje)
  };
}
          ...
          <div style={styles.headerLeft}>
            ...
            <div>
              ...
              <div style={styles.subtitle}>
                ...
                <span style={styles.statusWrap} title={
                  econ.status === "ok" ? "UF/UTM en vivo (mindicador.cl)" :
                  econ.status === "error" ? "Sin conexión — usando valores de respaldo" :
                  "Conectando…"
                }>
                  <span style={{ ...styles.statusDot, ...(econ.status === "ok" ? styles.statusDotOk : econ.status === "error" ? styles.statusDotError : styles.statusDotLoading) }} />
                  {econ.status === "ok" && "en vivo"}
                  {econ.status === "loading" && "conectando…"}
                  {econ.status === "error" && "sin conexión"}
                </span>
              </div>
            </div>
          </div>
          ...
          {result && result.bajarTramo && (
            <div style={styles.bajarTramoBox}>
              ...
              <div style={styles.bajarTramoRow}>
                ...
                <span style={styles.bajarTramoValue}>{result.bajarTramo.topado ? "No alcanzable con el tope legal" : pct(result.bajarTramo.tramoDestino)}</span>
              </div>
              ...
              {result.bajarTramo.topado && (
                <div style={styles.bajarTramoAlerta}>
                  ⚠ Para bajar de tramo se necesitarían {uf(result.bajarTramo.apvNecesarioUFReal)} mensuales, pero el Régimen B tiene un tope legal de 600 UF anuales (50 UF/mes). Con el máximo permitido se obtiene la mayor rebaja posible dentro del mismo tramo, pero no es suficiente para cruzar al tramo anterior.
                </div>
              )}
            </div>
          )}
...
const styles = {
  ...
  subtitle: { color: "#8a929d", fontSize: 13, marginTop: 2, display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" },
  statusWrap: { display: "inline-flex", alignItems: "center", gap: 5, fontSize: 11.5, color: "#6f7785" },
  statusDot: { width: 7, height: 7, borderRadius: "50%", display: "inline-block" },
  statusDotOk: { background: "#3ecf6e", boxShadow: "0 0 6px rgba(62,207,110,0.7)" },
  statusDotLoading: { background: "#e3b341", animation: "pulse 1s ease-in-out infinite" },
  statusDotError: { background: "#e07a5f" },
  ...
  bajarTramoAlerta: { fontSize: 12, color: "#dbb27a", background: "#1a1410", border: "1px solid #3a2f1f", borderRadius: 8, padding: "8px 10px", marginTop: 10, lineHeight: 1.5 },
